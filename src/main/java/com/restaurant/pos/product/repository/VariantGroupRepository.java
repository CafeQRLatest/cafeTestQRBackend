...
public interface VariantGroupRepository extends JpaRepository<VariantGroup, UUID> {
    ...
    @Query("SELECT v FROM VariantGroup v WHERE v.clientId = :clientId AND (:orgId IS NULL OR v.orgId IS NULL OR v.orgId = :orgId)")
    List<VariantGroup> findByClientIdAndOrgIdOrGlobal(UUID clientId, UUID orgId);

    @EntityGraph(attributePaths = {"options"})
    @Query("SELECT v FROM VariantGroup v WHERE v.clientId = :clientId AND (:orgId IS NULL OR v.orgId IS NULL OR v.orgId = :orgId) AND v.isActive = true")
